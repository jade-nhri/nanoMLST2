...
def marker (infile,download):
    d=dict()
    if download==True:
        if os.path.exists(infile+".fasta"):
            subprocess.getoutput('rm '+infile+".fasta")
        getf="wget https://rest.pubmlst.org/db/pubmlst_saureus_seqdef/loci/{0}/alleles_fasta -O {0}.fasta".format(infile)
        print (getf)
        subprocess.getoutput(getf)
        subprocess.getoutput("rm saureus.txt")
        print ("wget https://rest.pubmlst.org/db/pubmlst_saureus_seqdef/schemes/1/profiles_csv -O saureus.txt")
        subprocess.getoutput("wget https://rest.pubmlst.org/db/pubmlst_saureus_seqdef/schemes/1/profiles_csv -O saureus.txt")
    if not os.path.exists(infile+".fasta"):
        getf="wget https://rest.pubmlst.org/db/pubmlst_saureus_seqdef/loci/{0}/alleles_fasta -O {0}.fasta".format(infile)
        print (getf)
        subprocess.getoutput(getf)
    ...
    if not os.path.exists(infile+'.fasta'):
        f=open(infile+'.fasta')
        fw=open(infile+'_f.fasta','w')
        fw1=open(infile+'_excluded.fasta','w')
        with f as fp:
...
def correct (fafile,gene):
    comm="makeblastdb -in {0}.fasta -dbtype nucl".format(gene)
    subprocess.getoutput(comm)
    comm="blastn -query {0} -db {1}.fasta -out temp.result -max_target_seqs 10 -num_threads 1 -outfmt '7 qseqid stitle qlen slen length qstart qend sstart send pident evalue qseq sseq'".format(infile,gene)
    subprocess.getoutput(comm)
    f=open('temp.result')
